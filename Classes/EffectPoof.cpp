...
EffectPoof::EffectPoof(Scene * scene, Vec2 position, MonsterKind monsterkind)
{
	...
	case MonsterKind_Clotty:
	{
		...
		EffectPoofSprite->runAction(Sequence::create(Spawn::createWithTwoActions(EffectPoofAnimate, CallFunc::create(CC_CALLBACK_0(EffectPoof::CreateClotty, this, scene, position))), RemoveSelf::create(), nullptr));
	}
		break;
	default:
		break;
...
void EffectPoof::CreateClotty(Scene * scene, Vec2 position)
{
	ClottyMaker = new Clotty(scene, position);
}

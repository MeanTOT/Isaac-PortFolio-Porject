...
MagicMushRoom::MagicMushRoom(Scene * scene, Vec2 position)
{
	cache = SpriteFrameCache::getInstance();

	itemSprite = Sprite::create("ITEMS/collectibles_012_magicmushroom.png");
...
void MagicMushRoom::SetZorder()
{
	itemSprite->setLocalZOrder(itemSprite->getPositionY() * -1 + 100);
}

void MagicMushRoom::EraseItem()
{
	if (itemSprite->getTag() == ItemErase)
	{
		...
		Player->doGetItemAction(MagicMushRoomITEM);
	}
}
